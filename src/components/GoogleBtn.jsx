...
export default function GoogleCustomBtn({ page = "signin", showProfile = false }) {
  ...
  const login = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      ...
      setLoading(true);
      try {
        // First, check if user exists and needs account type selection
        const checkResponse = await client.post("/auth/google/check-status", {
          accessToken: tokenResponse.access_token
        });

        const data = checkResponse.data;

        if (data.requiresAccountType) {
  ...
  const handleAccountTypeSuccess = () => {
    // This will be called when account creation is successful
  ...
  return (
    <>
      ...
      <AccountTypeModal
        ...
        onSuccess={handleAccountTypeSuccess}
      />
    </>
  );
}
