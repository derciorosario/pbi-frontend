          ...
          {loading ? (
            ...
            <>
              ...
              <div className="flex gap-3 mt-6 relative">
                ...
                {/* Share Menu */}
                {shareOpen && <ShareMenu experience={experience} shareMenuRef={shareMenuRef} setShareOpen={setShareOpen} />}
              </div>
            </>
          )}
        </div>
...
const ShareMenu = ({ experience, shareMenuRef, setShareOpen }) => {
  const shareUrl = `${window.location.origin}/experience/${experience?.id}`;
  const shareTitle = experience?.title || "Experience on 54Links";
  const shareQuote = (experience?.description || "").slice(0, 160) + ((experience?.description || "").length > 160 ? "…" : "");
  const shareHashtags = ["54Links", "Travel", "Experiences"].filter(Boolean);
