...
const REDIRECT_DELAY_MS = 5000; // delay before redirecting after success

export default function DeleteAccount() {
  const { token } = useParams();
  const navigate = useNavigate();
  const { logout } = useAuth();
  const [status, setStatus] = useState("deleting"); // "deleting" | "success" | "error"
  ...
  useEffect(() => {
    async function run() {
      try {
        ...
        setStatus("success");
        setMessage("Your account has been permanently deleted. Thank you for using our platform.");
        ...
        setStatus("error");
        setMessage(
          "This deletion link is no longer valid—it may have expired or already been used. " +
          "If you still want to delete your account, please request a new deletion email from your settings."
        );
      }
    }
      ...
      <div className="w-full max-w-xl">
        ...
        >
          ...
          <p className="mt-3 text-center text-base sm:text-lg text-slate-600">
            {message || (status === "deleting" ? "Please wait while we permanently delete your account and all associated data." : "")}
          </p>
